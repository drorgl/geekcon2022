...
const int pwmPIN[] = {2, 4}; // an array to identify the PWM input pins (the array can be any length)
...
int RC_decode(int CH)
{
  ...
  {                    // If the RC signal is valid
    CH_output = PW[i]; // calibrate(PW[i], Min, Mid, Max); // calibrate the pulse width to the range -1 to 1.
  }
  return CH_output;
